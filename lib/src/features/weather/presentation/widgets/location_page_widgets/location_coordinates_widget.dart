...
class LocationCoordinatesWidget extends StatelessWidget {
  const LocationCoordinatesWidget({Key? key}) : super(key: key);
    ...
    return Column(
      ...
      children: [
        BlocSelector<LocationCubit, LocationState, String?>(
          selector: (state) {
            return state is LocationTypeFormValidatingErrorState &&
                    state.errMessage.contains('Latitude')
                ? state.errMessage
                : null;
          },
          builder: (context, state) {
            return Container(
              ...
              child: TextFormField(
                controller: LocationCubit.get(context).latController,
        ...
        BlocSelector<LocationCubit, LocationState, String?>(
          selector: (state) {
            return state is LocationTypeFormValidatingErrorState &&
                    state.errMessage.contains('Longitude')
                ? state.errMessage
                : null;
          },
          builder: (context, state) {
            return Container(
              margin: const EdgeInsets.symmetric(horizontal: 10),
              child: TextFormField(
                controller: LocationCubit.get(context).lonController,
                keyboardType: TextInputType.number,
                maxLines: null,
                autofocus: false,
                style: const TextStyle(
